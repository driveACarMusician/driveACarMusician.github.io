...
class Level3 extends Phaser.Scene {
  ...
  create() {
    ...
    //map generation
    //spawn x amount of random coins and obstacles with x = gameObjectlvl3 variable
    for (var i = 0; i <= gameObjectlvl3; i++) {
      let randomValue = Math.random();
      let YvalueNew = Yvalue - i * obstacleGap;
      let randomx = positions[Math.floor(Math.random() * positions.length)];
      if (i < gameObjectlvl3) {
        // console.log(i, gameObjectlvl3, randomValue);
        if (randomValue <= 0.2) {
          //make coin
  ...
  moveObstacle(obstacle, speedlvl3) {
    obstacle.y += speedlvl3;
  ...
  update() {
    //make coins obstacles and finish line move at the same speedlvl3 as the background
    ...
    //moving background
    this.background.tilePositionY -= speedlvl3;
